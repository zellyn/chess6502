...
// TestDebugDepthGame: one carryover game, 0x1F (A, White) vs 0x0F (B),
// logging realized depth, abort state, and cycles per move — the
// diagnostic for the TSCP-match depth collapse (self-play SPRT hides
// an absolute regression that hits both sides equally).
func TestDebugDepthGame(t *testing.T) {
	...
	for ply := 0; ply < 60; ply++ {
		...
		exited, code, err := m.Run(budget*30 + 2_000_000_000)
		...
		t.Logf("ply %2d %s(%#02x): %-6s depth=%d abort=%d score=%6d cycles=%3dM",
			ply, map[bool]string{true: "A", false: "B"}[aTurn], features, ms,
			m.Mem.Main[defs["CURDEPTH"]], m.Mem.Main[defs["ABORT"]],
			score, m.Cycles/1_000_000)
